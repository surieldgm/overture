...
class OvertureUiApp:
    ...
    def _handle_export_post(self, environ: dict[str, object], start_response: StartResponse) -> Iterable[bytes]:
        ...
        session = session_from_environ(environ)
        if action == "back":
            return self._redirect(start_response, "/ticket", extra_headers=[("Set-Cookie", _session_cookie(session))])
...
def build_ui_server(
    ...
    *,
    ...
    app = OvertureUiApp(store_dir=store_dir, llm_client=llm_client, linear_client_factory=linear_client_factory)
    return make_server(
        host,
        port,
        app,
...
def prepare_export_review(
    ...
    *,
    ...
    if session_ticket_path is not None:
        try:
            parsed = parse_ticket_file(session_ticket_path)
        except ValueError as exc:
            return ExportReviewResult(session=next_session, ticket_path=session_ticket_path, message=str(exc))
        ...
        return ExportReviewResult(
            ...
            ticket_path=session_ticket_path,
            title=parsed.title,
    ...
    try:
        ...
        return ExportReviewResult(session=next_session, ticket_path=ticket_path, message=str(exc))
...
def render_export_page(review: ExportReviewResult) -> str:
    ...
    if review.ticket_path is None or not review.title:
        ...
        actions = '<div class="form-footer"><a href="/ticket">Back to ticket review</a></div>'
    else:
        summary_markup = f"""
